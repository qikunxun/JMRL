...
def train(args, model, train_features, dev_features):
    ...
    optimizer_grouped_parameters = [
        {"params": [p for n, p in model.named_parameters() if not any(nd in n for nd in new_layer + diff_layer)], },
        {"params": [p for n, p in model.named_parameters() if any(nd in n for nd in new_layer)], "lr": args.lr_added},
        {"params": [p for n, p in model.named_parameters() if any(nd in n for nd in diff_layer)], "lr": 1e-2},
    ]
...
def main():
    ...
    save_path_ = os.path.join(args.save_path, time)
    ...
    if args.do_train:  # Training
        ...
        if args.eval_mode != "fushion":
            ...
            score_path = os.path.join(args.load_path, "{}_scores.csv".format(basename))
            ...
            print({"dev_F1": merged_re[-1] * 100, "dev_evi_F1": merged_evi[-1] * 100, "dev_F1_ign": merged_re_ign[-1] * 100})
