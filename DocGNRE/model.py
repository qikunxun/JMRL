...
class DocREModel(nn.Module):
    ...
    def __init__(self, config, model, tokenizer,
                 ...
                 max_sent_num=25, evi_thresh=0.2, T=2, L=20):
        '''
    ...
    def reasoning_by_soft_rules(self, logits):
        ...
        # input = F.one_hot(input, self.n).float()
        input = logits[:, :, :]
        # input_nr = logits[:, :, 0:1]
        # input = input.view(n_e, n_e, self.n)
        input = torch.cat([input, torch.permute(input, (1, 0, 2)), torch.unsqueeze(eye, dim=-1)], dim=-1)
        # input = torch.cat([input, torch.unsqueeze(eye, dim=-1)], dim=-1)
        all_states = []
        ...
        # output = torch.cat([input_nr, output], dim=-1)
    ...
    def forward(self,
        ...
        for b in range(len(hts)):
            ...
            # input = torch.cat([input[:, 0:1], input[:, 1:] * (1 - input[:, 0:1])], dim=-1)
            ...
            start = end
        # logits_gate = torch.sigmoid(self.proj(logits))
        logits_rule_soft_tmp = torch.cat(logits_rule_soft, dim=0)
        logits_rule_soft = logits_rule_soft_tmp + logits

        output['logits'] = logits_rule_soft
        output["rel_pred"] = self.loss_fnt.get_label(logits_rule_soft, num_labels=self.num_labels)
        ...
        if tag in ["test", "dev"]:  # testing
            scores_topk = self.loss_fnt.get_score(logits_rule_soft, self.num_labels)
        ...
        #     output["attns"] = doc_attn.split(batch_rel)
            ...
            loss = self.loss_fnt(logits, labels.float()) + self.loss_fnt(logits_rule_soft.float(), labels.float())
        ...
        if "loss" in output: print(output["loss"])
        return output
