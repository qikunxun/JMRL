...
class GAIN_BERT(nn.Module):
    ...
    def forward(self, **params):
        ...
        outputs = self.bert(input_ids=words, attention_mask=mask)
        encoder_outputs = outputs[0]
        sentence_cls = outputs[1]
        ...
        for i in range(len(graphs)):
            ...
            if features is None:
                ...
                features = torch.cat((features, x), dim=0)
        graph_big = dgl.batch_hetero(graphs)
        if torch.cuda.is_available(): graph_big = graph_big.to('cuda')
        output_features = [features]
        for GCN_layer in self.GCN_layers:
            features = GCN_layer(graph_big, {"node": features})["node"]  # [total_mention_nums, gcn_dim]
            output_features.append(features)
        ...
        # entity_graphs = params['entity_graphs']
        # entity_graph_big = dgl.batch(entity_graphs)
        # if torch.cuda.is_available(): entity_graph_big = entity_graph_big.to('cuda')
        # self.edge_layer(entity_graph_big, entity_graph_feature)
        #
        # entity_graphs = dgl.unbatch(entity_graph_big)
        # path_info = get_cuda(torch.zeros((bsz, h_t_limit, self.gcn_dim * 4)))
        ...
        n_e = entity_num
        relation_mask = params['relation_mask']
        predictions_new = torch.zeros_like(predictions)
        for b in range(h_t_pairs.shape[0]):
            if relation_mask is not None: mask = relation_mask[b]
            indices = h_t_pairs[b].transpose(1, 0).to(predictions.device)
            input = torch.sigmoid(predictions[b])
            if relation_mask is not None: input = input * mask.unsqueeze(dim=-1)
            matrix = torch.sparse.FloatTensor(indices.long(), input,
                                              torch.Size([n_e, n_e, self.config.relation_nums])).to(predictions.device)
            logits_rule = self.reasoning_by_soft_rules(matrix.to_dense())
            logits_rule = logits_rule.view(-1, self.config.relation_nums)
            indices = indices[0] * n_e + indices[1]
            logits_rule = logits_rule[indices.long()]
            predictions_new[b] = logits_rule

        logits_rule_soft = predictions_new + predictions

        return predictions, logits_rule_soft
