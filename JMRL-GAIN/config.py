...
data_dir = '../dataset_dwie/'
prepro_dir = os.path.join(data_dir, 'prepro_data/')
if not os.path.exists(prepro_dir):
    os.mkdir(prepro_dir)

rel2id = json.load(open(os.path.join(data_dir, 'meta/rel2id.json'), "r"))
id2rel = {v: k for k, v in rel2id.items()}
word2id = json.load(open(os.path.join(data_dir, 'meta/word2id.json'), "r"))
ner2id = json.load(open(os.path.join(data_dir, 'meta/ner2id.json'), "r"))

word2vec = np.load(os.path.join(data_dir, 'meta/vec.npy'))
...
def get_opt():
    ...
    parser.add_argument('--relation_nums', type=int, default=66)
    parser.add_argument('--entity_type_num', type=int, default=10)
    parser.add_argument('--max_entity_num', type=int, default=200)
    ...
    parser.add_argument('--lambda_al', type=float, default=1.0)
