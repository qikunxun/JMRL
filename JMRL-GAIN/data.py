...
class BERTDGLREDataset(IterableDataset):
    ...
    def create_graph(self, Ls, mention_id, entity_id, entity2mention):
        ...
        if d[('node', 'intra', 'node')] == []:
            d[('node', 'intra', 'node')].append((entity2mention[min(entity2mention.keys())][0], 0))
...
class DGLREDataloader(DataLoader):
    ...
    def __init__(self, dataset, batch_size, shuffle=False, h_t_limit_per_batch=300, h_t_limit=10000, relation_num=66,
            ...
            for i, example in enumerate(minibatch):
                ...
                if self.dataset_type == 'train':
                    ...
                    for j, (h_idx, t_idx) in enumerate(train_tripe):
                        ...
                        delta_dis = max(min(hlist[0]['global_pos'][0] - tlist[0]['global_pos'][0], 511), -511)
                        if delta_dis < 0:
