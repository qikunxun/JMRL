...
class Config(object):
	...
	def get_train_batch(self):
		...
		ht_pairs = torch.LongTensor(self.batch_size, self.h_t_limit, 2).to(self.device)
		...
		for b in range(self.train_batches):
			...
			for i, index in enumerate(cur_batch):
				...
				for j, (h_idx, t_idx) in enumerate(train_tripe):
					...
					ht_pairs[i, j, :] = torch.Tensor([h_idx + 1, t_idx + 1]).to(self.device)

					delta_dis = max(min(hlist[0]['pos'][0] - tlist[0]['pos'][0], self.max_length - 1), 1 - self.max_length)
					if delta_dis < 0:
	...
	def get_test_batch(self):
		...
		for b in range(self.test_batches):
			...
			for mapping in [h_mapping, t_mapping, relation_mask, ht_pairs]:
				mapping.zero_()
				...
				for h_idx in range(L):
					for t_idx in range(L):
						if h_idx != t_idx:
							...
							delta_dis = max(min(hlist[0]['pos'][0] - tlist[0]['pos'][0], self.max_length - 1),
											1 - self.max_length)
	...
	def train(self, model_pattern, model_name):
		...
		optimizer = optim.Adam([
			{'params': [p for n, p in model.named_parameters() if not any(nd in n for nd in diff_layer)],
			 'weight_decay': self.weight_decay},
			{"params": [p for n, p in model.named_parameters() if any(nd in n for nd in diff_layer)], "lr": 1e-1},
		], lr=self.lr)
		...
		for epoch in range(self.max_epoch):
			...
			for data in self.get_train_batch():
				...
				h_t_paris = data['h_t_pairs']

				dis_h_2_t = ht_pair_pos+10
				dis_t_2_h = -ht_pair_pos+10


				predict_re, predict_re_rule = model(context_idxs, context_pos, context_ner, context_char_idxs, input_lengths, h_t_paris, h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h)
				loss_1 = torch.sum(BCE(predict_re, relation_multi_label)*relation_mask.unsqueeze(2)) /  (self.relation_num * torch.sum(relation_mask))
				loss_2 = torch.sum(BCE(predict_re_rule, relation_multi_label)*relation_mask.unsqueeze(2)) / (self.relation_num * torch.sum(relation_mask))
				loss = loss_1 + loss_2
	...
	def test(self, model, model_name, output=False, input_theta=-1):
		...
		for data in self.get_test_batch():
			with torch.no_grad():
				...
				predict_re, predict_re_rule = model(context_idxs, context_pos, context_ner, context_char_idxs, input_lengths, h_t_pairs,
								   h_mapping, t_mapping, relation_mask, dis_h_2_t, dis_t_2_h)

				predict_re = torch.sigmoid(predict_re_rule)
